...
if ( ! function_exists( 'my_calendar_settings_page_init' ) ) {
    ...
    // ... (All other functions from the original file are below) ...
    ...
    /**
     * NEW: This function runs on the special 'load-...' hook.
    ...
    function my_calendar_load_events_list_table_screen() {
        // We need the posts list table class
        require_once( ABSPATH . 'wp-admin/includes/class-wp-posts-list-table.php' );
        
        // Trick WordPress into thinking we are on the standard 'edit.php' page for events.
        global $typenow, $current_screen;
        $typenow = 'event';
        set_current_screen( 'edit-event' ); // This sets the screen ID, which is vital for many hooks.

        // Create the list table object and store it globally so our render function can find it.
        global $my_calendar_list_table;
        $my_calendar_list_table = new WP_Posts_List_Table( [ 'screen' => get_current_screen() ] );
        
        // This makes the "Screen Options" tab work correctly (e.g., "Number of items per page").
     ...
     * The heavy lifting has already been done in the 'load-...' hook function.
     */
    function my_calendar_render_all_events_page() {
        global $my_calendar_list_table; // Get the list table object we prepared earlier.
        
        // Now, we prepare and display the items.
    ...
    // --- (The rest of the file is unchanged, but included fully) ---

    function my_calendar_get_default_colors() {
        return [ 'default_bg_color' => '#f7f7f7', 'default_text_color' => '#000000', 'scroll_date_bg_color' => '#a7b59a', 'scroll_date_text_color' => '#222222', 'grid_header_bg_color' => '#a7b59a', 'grid_header_text_color' => '#222222', 'grid_day_cell_bg_color' => '#f7f7f7', 'grid_day_number_text_color' => '#333333', 'list_day_cell_bg_color' => '#f7f7f7', 'list_day_number_text_color' => '#555555', 'spc_year_text_color' => '#333333', 'spc_dow_bg_color' => '#a7b59a', 'spc_dow_text_color' => '#222222', 'spc_info_bg_color' => '#c0cfb2', 'spc_info_text_color' => '#222222', 'spc_date_bg_color' => '#d1dacb', 'spc_date_text_color' => '#333333', ];
    }
    ...
    function my_calendar_render_settings_page() {
        ...
        $options = get_option( 'my_calendar_settings', [] ); $defaults = my_calendar_get_default_colors(); $global_colors = wp_parse_args( $options['global'] ?? [], $defaults ); $custom_styles = $options['styles'] ?? [];
        ?>
        <div class="wrap">
            <h1><?php echo esc_html( get_admin_page_title() ); ?></h1><p>Manage the global and custom color styles for your event calendars.</p>
                ...
                <div id="settings-container" style="display: flex; gap: 30px;">
                    <div class="settings-main-column" style="flex: 2;">
                        <div class="settings-section" id="global-settings-section">
                            <nav class="nav-tab-wrapper"><a href="#tab-scroll" class="nav-tab nav-tab-active">Scroll View</a><a href="#tab-grid" class="nav-tab">Grid View</a><a href="#tab-list" class="nav-tab">List View</a><a href="#tab-single-play" class="nav-tab">Single Play Calendar</a><a href="#tab-common" class="nav-tab">Common Event Colors</a></nav>
                        ...
                        <div class="settings-section">
                            <h2>Custom Calendar Styles</h2><p>Create named styles that inherit the global defaults. You only need to change the colors you want to override.</p>
                            <div id="custom-styles-container">
                                <?php if ( ! empty( $custom_styles ) ) : foreach ( $custom_styles as $id => $style ) : ?>
                                <div class="style-block" id="style-block-<?php echo esc_attr( $id ); ?>"><h3 class="style-title"><?php echo esc_html( $style['style_name'] ?? 'Unnamed' ); ?> <button type="button" class="button button-link-delete delete-style-button">Delete</button></h3><p><strong>Shortcode:</strong> <code>[monthly_events_calendar id="<?php echo esc_attr( $id ); ?>"]</code></p><?php my_calendar_render_color_fields_subset( 'styles', $style, array_keys($defaults), null, $id ); ?></div>
                                <?php endforeach; endif; ?>
                            </div>
                            <button type="button" id="add-new-style" class="button button-secondary">Add New Custom Style</button>
                        </div>
                    </div>
                    <div class="settings-side-column" style="flex: 1;"><div id="calendar-preview-pane" style="position: sticky; top: 50px;"><h2>Live Preview</h2><div id="calendar-preview-wrapper"></div></div></div>
                </div>
                <?php submit_button( 'Save All Settings' ); ?>
            </form>
        </div>
        <div id="new-style-template" style="display: none;"><?php my_calendar_render_color_fields_subset( 'styles', $defaults, array_keys($defaults), null, '__NEW_ID__', true ); ?></div>
    ...
    function my_calendar_render_color_fields_subset( $group, $values, $keys_to_render, $heading = null, $id = null, $is_template = false ) {
        ...
        <table class="form-table">
            ...
            <?php endif; ?>
            <?php foreach ($keys_to_render as $key): $label = $all_fields[$key]; $value = $values[$key] ?? ''; $default_color = $defaults[$key]; ?>
            <tr><th scope="row"><label for="<?php echo esc_attr("{$group}-{$id}-{$key}"); ?>"><?php echo esc_html($label); ?></label></th><td><div class="color-picker-wrap"><input type="text" id="<?php echo esc_attr("{$group}-{$id}-{$key}"); ?>" name="<?php echo esc_attr($base_name); ?>[<?php echo esc_attr($key); ?>]" class="wp-color-picker" value="<?php echo esc_attr($value); ?>" data-default-color="<?php echo esc_attr($default_color); ?>"><a href="#" class="reset-color-btn" title="Reset to default">↺</a></div></td></tr>
            <?php endforeach; ?>
        </table>
        <?php
    ...
    function my_calendar_sanitize_settings( $input ) {
        if ( function_exists('my_project_invalidate_calendar_cache') ) { my_project_invalidate_calendar_cache(); }
        $sanitized_output = []; $defaults = my_calendar_get_default_colors(); $color_keys = array_keys($defaults);
        if ( !empty( $input['global'] ) && is_array( $input['global'] ) ) { $sanitized_output['global'] = []; foreach ( $color_keys as $key ) { if ( isset( $input['global'][$key] ) ) { $sanitized_output['global'][$key] = sanitize_hex_color( $input['global'][$key] ); } } }
        if ( !empty( $input['styles'] ) && is_array( $input['styles'] ) ) { $sanitized_output['styles'] = []; foreach ( $input['styles'] as $id => $style_data ) { if ( empty( $style_data['style_name'] ) ) continue; $style_name = sanitize_text_field( $style_data['style_name'] ); $current_id = ( strpos($id, '__NEW_ID__') === 0 ) ? sanitize_title( $style_name ) : $id; if ( isset( $sanitized_output['styles'][$current_id] ) ) { $current_id .= '-' . time(); } $sanitized_output['styles'][$current_id]['style_name'] = $style_name; foreach ( $color_keys as $key ) { if ( isset( $style_data[$key] ) ) { $sanitized_output['styles'][$current_id][$key] = sanitize_hex_color( $style_data[$key] ); } } } }
